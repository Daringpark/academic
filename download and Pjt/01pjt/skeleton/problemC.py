...
def get_deposit_products():
  ...
  result = result.get('result').get('optionList')

  for product in result :
    new_product_dictionary = {}
    new_product_dictionary['금융상품코드'] = product['fin_prdt_cd']
    new_product_dictionary['저축 금리'] = product['intr_rate']
    new_product_dictionary['저축 기간'] = product['save_trm']
    new_product_dictionary['저축금리유형'] = product['intr_rate_type']
    new_product_dictionary['저축금리유형명'] = product['intr_rate_type_nm']
    new_product_dictionary['최고 우대금리'] = product['intr_rate2']
    whole_list.append(new_product_dictionary)
  result = whole_list
  return result
